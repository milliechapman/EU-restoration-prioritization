...
rij <- read_fst("data/formatted-data/features_split.fst") |>
  ...
  filter(species %in% c(24980270110, 999999, 52311900110))
...
###### solve for all nuts2 to find infeas ##########
...
for(k in 1:length(infeas_nuts2$nuts)){
  ...
  for(i in 1:length(nuts2_names)) { #length(nuts2_AT_names)
    nuts2 <- nuts2_names[[i]]
    adj <- ifelse(nuts2 %in% infeas_nuts2$nuts, 0.8, 0.98)
  ...
  tryCatch(solve(p_solve), error = function(e) { skip_to_next <<- TRUE})
  infeas_2$TF[n] <- skip_to_next
  infeas_2$nuts[n] <- nuts2_names
  
  write_csv(infeas_2, "data/feasibility-tests/nuts2_feasible_REF_2.csv", append = FALSE)
  n = n+1
  print(infeas_2)
  if(skip_to_next) { next } 
}
...
#### refine for final infeasable #############
#### 
infeas_nuts2 <- read_csv("data/feasibility-tests/nuts2_feasible_REF_2.csv") |>
  filter(TF == "TRUE")
...
for(k in 1:length(infeas_nuts2$nuts)){
  ...
  tryCatch(solve(p_solve), error = function(e) { skip_to_next <<- TRUE})
  infeas_3$TF[n] <- skip_to_next
  infeas_3$nuts[n] <- nuts2_names
  
  write_csv(infeas_3, "data/feasibility-tests/nuts2_feasible_REF_3.csv", append = FALSE)
  n = n+1
  print(infeas_3)
  if(skip_to_next) { next } 
}
...
infeas_nuts2_80 <- read_csv("data/feasibility-tests/nuts2_feasible_REF_2.csv") |>
  filter(TF == "TRUE")

infeas_nuts2_50 <- read_csv("data/feasibility-tests/nuts2_feasible_REF_3.csv") |>
  filter(TF == "TRUE")

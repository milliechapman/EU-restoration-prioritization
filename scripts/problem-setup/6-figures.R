...
################### figure 1 - conceptual ##########################
solution <- read_csv("data/solutions/sol/sol_carbon_0.5_restoration_0.141_production_TRUE_country_EVEN_wetlands_TRUE_onlyrestoration_FALSE_scenario_HN_globiom_IC_f455.csv") |>
...
fig3d_data <- plotting_data |>
  ...
  filter(action == "restore") |>
  mutate(diff = `FALSE`-`TRUE`) |> drop_na() |>
...
burden_biodiv <- rep_formatted |>
  ...
  mutate(country_contraint = ifelse(country_contraint == "TRUE", "I", "II"))
  
  # Box plot facetted by "dose"
burden_biodiv_plot <- ggboxplot(burden_biodiv, x = "country_contraint", y = "perc_met_rel",
  ...
  theme_classic() +
  scale_color_manual(values = c( "grey", "grey2")) +
  theme(legend.position = "none") + 
...
test <- rep |> dplyr::select(-c(column_label, rep, target, shortfall)) |>
  filter(species != 999999) |>
  mutate(taxon_id = ifelse(species > 10000000000,
                          substr(species, start = 1, stop = 7),
                          substr(species, start = 1, stop = 6)),
...
test |>
  mutate(target_met = ifelse(shortfall_perc > 0, 1, 0)) |>
  mutate(target_unmet = ifelse(shortfall_perc < 0, 1, 0)) |>
  group_by(country_contraint, scenario, carbon_weight, speciesgroup) |>
  summarise(target_met = sum(target_met, na.rm = TRUE),
            target_unmet = sum(target_unmet, na.rm = TRUE)) |>
  filter(carbon_weight == 0) |>
  ggplot(aes(x = speciesgroup,
             y = target_met/target_unmet, 
             fill = scenario,
             alpha = country_contraint)) +
  geom_col(position = "dodge") +
 # facet_wrap(~country_contraint) + 
  theme_minimal() +
  coord_flip()
...
test |>
  ...
  summarise(target_met = sum(target_met, na.rm = TRUE),
            target_unmet = sum(target_unmet, na.rm = TRUE)) |>
  #filter(carbon_weight == 0 | carbon_weight == 1) |>
  mutate(met_to_unmet = target_met/target_unmet) |>
  ggplot(aes(x = speciesgroup,
             y = met_to_unmet, 
             fill = scenario)) +
  geom_boxplot() +
  facet_grid(cols = vars(country_contraint)) + 
  stat_compare_means(aes(label = ..p.signif..)) +
  theme_bw() +
  coord_flip()
...
t <-test |>
  ...
  mutate(target_unmet = ifelse(shortfall_perc < 0, 1, 0)) |>
  group_by(taxon_id) |>
  mutate(all_met = sum(target_unmet, na.rm = TRUE)) |>
  ungroup() |>
  mutate(all_met= ifelse(all_met > 0,0,1)) |>
  group_by(country_contraint, scenario, carbon_weight, all_met) |> count()
  summarise(target_met = sum(target_met, na.rm = TRUE),
            target_unmet = sum(target_unmet, na.rm = TRUE)) 

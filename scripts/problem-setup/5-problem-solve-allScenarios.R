  ...
  # define production scenario
    if(future == "f455"){
      ...
      country_prod_constraints <- read_csv("data/formatted-data/linear_constraints/country_constraints_55_proportional.csv")|>
        mutate(value = ifelse(country %in% c("CY"), 0,value))
    ...
    if(future == "ref"){
      ...
      country_prod_constraints <- read_csv("data/formatted-data/linear_constraints/country_constraints_ref_proportional.csv")

      # deal with in-feasible nuts2

      infeas_nuts2 <- read_csv("data/feasibility-tests/nuts2_feasible_ref_adj.csv") |>
        mutate(adj = ifelse(TF =="FALSE", 1,0))
...
#### solve restoration only
manual_bounded_constraints_onlyrest <-
  manual_bounded_constraints |>
  mutate(upper = ifelse(zone %in% c("z11","z12","z13","z14",
                                    "z15","z16","z17","z18",
                                    "z19","z20", "z21", "z22",
                                    "z23", "z24", "z25"),0, upper)) |>
  mutate(lower = ifelse(zone %in% c("z11","z12","z13","z14",
                                    "z15","z16","z17","z18",
                                    "z19","z20", "z21", "z22",
                                    "z23", "z24", "z25"),0, lower))
...
problem_setup(
  ...
  pu, # pu data (x from p)
  targs,
  manual_bounded_constraints = manual_bounded_constraints_onlyrest,
...
scenarios <-
  tidyr::crossing(# Do all combinations of:
    carbon_weight = c(0.1,0.3,0.7,1.1,1.3,1.7,1.9),#c(0.5,0.9,1.5,2),#c(0.1, 0.3,seq(0.5,2, by = 0.5)),
